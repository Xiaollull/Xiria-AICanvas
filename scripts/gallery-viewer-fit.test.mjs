...
function fixture(cssHref) {
  return `<!doctype html><html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><link rel="stylesheet" href="${cssHref}"><style>:root{--lime:#8a73ff;--accent-rgb:138 115 255}.gallery-focus-shell.fixture-shell{position:static;width:100%;height:auto;display:block;margin:0;overflow:visible;padding:0;border:0;background:transparent;box-shadow:none}.fixtures{display:grid;grid-template-columns:1fr 1fr;gap:20px;padding:20px}.fixture-stage{width:calc(50vw - 30px);height:700px}.fixture-stage .gallery-viewer-image-viewport{width:100%;height:100%;box-sizing:border-box}</style></head><body><div class="gallery-focus gallery-focus-shell fixture-shell"><div class="fixtures"><div class="fixture-stage"><div id="wide" class="gallery-viewer-image-viewport"><span class="gallery-focus-frame ready"><img class="gallery-focus-full" data-width="1200" data-height="200" src="${svg(1200, 200)}"></span></div></div><div class="fixture-stage"><div id="portrait" class="gallery-viewer-image-viewport"><span class="gallery-focus-frame ready"><img class="gallery-focus-full" data-width="200" data-height="1200" src="${svg(200, 1200)}"></span></div></div></div></div><script>const box=node=>{const r=node.getBoundingClientRect();return[r.left,r.top,r.right,r.bottom,r.width,r.height]};const visibleObjectBox=image=>{const r=image.getBoundingClientRect();const width=Number(image.dataset.width);const height=Number(image.dataset.height);const scale=Math.min(r.width/width,r.height/height);const visibleWidth=width*scale;const visibleHeight=height*scale;const left=r.left+(r.width-visibleWidth)/2;const top=r.top+(r.height-visibleHeight)/2;return[left,top,left+visibleWidth,top+visibleHeight,visibleWidth,visibleHeight]};const measure=id=>{const stage=document.getElementById(id);const frame=stage.querySelector('.gallery-focus-frame');const image=stage.querySelector('img');const stageStyle=getComputedStyle(stage);const frameStyle=getComputedStyle(frame);const imageStyle=getComputedStyle(image);return{stage:box(stage),frame:box(frame),image:visibleObjectBox(image),imageElement:box(image),stageStyle:[stageStyle.backgroundColor,stageStyle.backgroundImage],frameStyle:[frameStyle.width,frameStyle.height,frameStyle.maxWidth,frameStyle.maxHeight],imageStyle:[imageStyle.width,imageStyle.height,imageStyle.maxWidth,imageStyle.maxHeight,imageStyle.objectFit]}};document.documentElement.dataset.geometry=JSON.stringify({wide:measure('wide'),portrait:measure('portrait')});</script></body></html>`;
}
...
function assertContained(value, label) {
  ...
  assert.ok(Math.max(image[4] / stage[4], image[5] / stage[5]) > 0.94, `${label} wastes too much of the available stage; ${geometry}`);
