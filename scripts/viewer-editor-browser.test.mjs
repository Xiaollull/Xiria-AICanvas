...
function fixture({ sidebar, expanded }, editorSource) {
  ...
  :root{--n-01:#090a0c;--n-02:#111218;--n-03:#181923;--n-05:#252636;--n-06:#303146;--n-07:#3b3d55;--n-08:#4b4d68;--n-13:#7d8097;--n-15:#9295ad;--n-17:#a9acc3;--n-18:#b2b5ca;--n-20:#c1c4d8;--n-27:#ececf6;--n-28:#f3f1fa;--n-31:#faf9fd;--n-32:#fff;--lime:#c8acfb;--accent-rgb:200 172 251;--line:#303146;--z-immersive:100;box-sizing:border-box}*{box-sizing:border-box}html,body{margin:0;width:100%;height:100%;overflow:hidden}button,input,select{font:12px sans-serif}.image-viewer-backdrop{position:fixed;inset:0}.image-viewer.fixture{max-width:none;max-height:none;height:${expanded ? "100vh" : "calc(100vh - 36px)"}}.viewer-edge-panel .tall{height:900px}.viewer-history{min-height:0}
    ...
    <div class="viewer-toolbar-group viewer-editor-tools"><button aria-pressed="true"><span>移动</span></button><button><span>画笔</span></button><button><span>橡皮</span></button></div>
  ...
  <script>
    ...
    const rotated={naturalWidth:118,naturalHeight:54,x:0,y:0,scale:1,rotation:45}; const bounds=viewerEditorLayerBounds(rotated);
    ...
    const result={layout,paint:{brush:pixel(undone,8,16),erased:pixel(painted,16,16),undo:pixel(undone,16,16)},rotated:{rotation:rotated.rotation,bounds},budgetError,errors:fixtureErrors}; document.getElementById("result").textContent=JSON.stringify(result);
  ...
  try {
    await withServer(async (port) => {
      for (const scenario of [
        ...
        assert.equal(result.rotated.rotation, 45);
        assert.ok(result.rotated.bounds.width > 120 && result.rotated.bounds.height > 120, "a rotated layer's AABB must expand");
