...
test("layout is stored apart from every creation parameter, so a reset cannot reach one", async () => {
  ...
  assert.deepEqual([...new Set(keys)], ["WORKSPACE_LAYOUT_STORAGE_KEY"], "only the resizable text-to-image workspace retains a layout key");
...
test("the panel is resizable and collapsible from the gutter, and the grid keeps its shipped default", async () => {
  ...
  assert.match(styles, /\n\.workspace \{[^}]*grid-template-columns: minmax\(360px, 400px\) 7px minmax\(420px, 1fr\) minmax\(350px, 460px\)/);
