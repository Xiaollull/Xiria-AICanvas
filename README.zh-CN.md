...
只有当发布同时提供精确命名并已完成上传的 `XirAI-<version>.7z` 与 `XirAI-<version>.7z.sha256` 时才会启用更新下载。校验和文件会在启用任何归档线路前严格读取和验证；如果 API 提供摘要，还会进行交叉校验。缺失、格式错误、歧义、草稿、预发布或命名错误的 Release 会安全拒绝，不会显示成可用更新。发布源、仓库、镜像与可选令牌均可在 `.env` 中配置，参见 `.env.example`。

发布新版本时规则是严格的：推送符合 `v主版本.次版本.修订版本` 的稳定标签（例如 `v1.0.1`），并确保 `package.json` 与两个 lockfile 版本字段完全一致。Release 工作流会生成精确命名的 `XirAI-<version>.7z` 及对应 `.sha256` 附件，验证受限发布清单；存在上一稳定版本时，还会用它测试更新流程。首次 `v1.0.0` 使用隔离的首次发布校验。仓库尚未发布任何版本时会显示“已是最新版本”，而不是报错。
